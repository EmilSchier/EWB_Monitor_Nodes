...
bool RV3028::setCountdownTimer(uint16_t time, time_units unit, bool repeatmode)
{
	...
	case UNIT_M_SECOND:
		...
		{
			clockFreq = 0;
			timerVal = time/1000*4096;
		}else {
			clockFreq = 1;
			timerVal = time/1000*64;
		}
		break;
	default:
		return false;
	...
	uint8_t value = readRegister(RV3028_CTRL1);
	value &= ~(1 << CTRL1_TE); //Clear the Timer enable bit
	writeRegister(RV3028_CTRL1, value);
...
void RV3028::disableCountdownTimer()
{
	disableTimerInterrupt();
	uint8_t value = readRegister(RV3028_CTRL1);
	value &= ~(1 << CTRL1_TE); 
...
{
	uint8_t value = readRegister(RV3028_CTRL2);
	value |= (1 << CTRL2_TIE); //Set the interrupt enable bit
	writeRegister(RV3028_CTRL2, value);
}
...
{
	uint8_t value = readRegister(RV3028_CTRL2);
	value &= ~(1 << CTRL2_TIE); //Clear the interrupt enable bit
	writeRegister(RV3028_CTRL2, value);
}
/**************************** her stopper mine (Emil) tilføjelser *****/
